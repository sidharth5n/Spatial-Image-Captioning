...
class SelfCriticalReward(nn.Module):

    def __init__(self,
                 cider_reward_weight: float,
                 bleu_reward_weight: float):
        super().__init__()
    ...
    def forward(self, model, att_feats, att_masks, boxes, gts, gen_result):
        """
        ...
        for i in range(batch_size):
            res[i] = [self.array_to_str(gen_result[i])]
        for i in range(batch_size):
            res[batch_size + i] = [self.array_to_str(greedy_res[i])]

        gts_ = OrderedDict()
        for i in range(len(gts)):
            gts_[i] = [self.array_to_str(gts[i][j]) for j in range(len(gts[i]))]
